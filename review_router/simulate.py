...
Completion means the simulated reviewer finished the job. Human confirmation
is required for moderation; this simulation does not observe enforcement outcomes.
"""
...
# This is a reporting threshold, not a statistical guarantee about tail accuracy.
P99_MIN_SAMPLES = 100
...
class JobRecords:
    ...
    def from_rows(rows: list[dict[str, Any]], horizon_min: float) -> JobRecords:
        ...
        def risk_flag(value: Any) -> bool:
            if isinstance(value, str):
                if value.lower() in {"true", "1"}:
                    return True
                if value.lower() in {"false", "0"}:
                    return False
                raise ValueError(f"invalid high_risk flag {value!r}")
            if value not in (0, 1):
                raise ValueError(f"invalid high_risk flag {value!r}")
            return bool(value)
        ...
        return JobRecords(
            ...
            high_risk=np.array([risk_flag(r["high_risk"]) for r in rows], dtype=bool),
            harm=col("harm", float),
...
class SimResult:
    ...
    unfinished_age_p50: float | None  # time since arrival for unfinished jobs, at the horizon
