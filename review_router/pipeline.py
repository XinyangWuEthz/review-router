...
def _prevalence_shift(
    ...
    A calibrated model carries the training prevalence into its probabilities:
    the sum of p over the test rows is the count of positives it expects. When
    that exceeds the actual count the model over-predicts volume by that
    factor, which ROC-AUC cannot see because it is rank-based.
    """
...
def queue_inputs(
    ...
    """The queued-job table every simulation reads.

    Built from the same columns predictions.csv stores (probabilities, labels,
    final tier), so the reproducibility gate can rebuild it from a saved run
    and replay a scenario exactly.
    """
    ...
    section = {
        "assumptions": {
            **asdict(config.sim),
            ...
            "review_completion_means_action_completion": (
                "simulation assumption: a completed review counts as a completed action; "
...
def _headline(metrics: dict[str, Any], config: RunConfig) -> dict[str, Any]:
    """The one time metric the protocol selects, router versus FIFO on the same population.
    ...
    def read(strategy: str, field: str) -> tuple[float | None, int, bool]:
        block = metrics.get(_key(load, strategy), {}).get("pooled_over_seeds", {}).get(field, {})
        return block.get(f"p{pct}"), int(block.get("n", 0)), bool(block.get("p99_reliable", False))

    def compare(field: str) -> dict[str, Any]:
        r, n_r, rel_r = read(config.primary_strategy, field)
        f_, n_f, rel_f = read("fifo", field)
        out: dict[str, Any] = {
            "router": r,
            "fifo": f_,
            "n_router": n_r,
    ...
    return {
        ...
        "population": "every queued job at this load, pooled over seeds; unfinished jobs are "
        "counted in status, not timed",
        "selected": compare(metric),
        "supplementary_high_risk": {
            "note": "reported alongside, never a substitute for the selected metric",
            "wait_p90": compare("high_risk_wait") | {"percentile": 90}
            if pct != 90
            else compare("high_risk_wait"),
        },
    }
    ...
    if "time_metrics" in sim:
        ...
        lines += [
            ...
            + (
                ...
                "(FIFO value is zero, no percentage)."
            ),
            "",
...
def run(config_path: Path) -> Path:
    ...
    pd.DataFrame(job_rows).to_csv(run_dir / "simulation_jobs.csv", index=False)
