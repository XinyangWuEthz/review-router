...
def test_wilson_interval_thirty_of_thirty_does_not_prove_99_percent() -> None:
    low, high = wilson_interval(30, 30)
    assert high == 1.0
    assert low < 0.99, "30/30 is compatible with precision well below 0.99"
...
def test_tier_metrics_marks_small_counts_as_not_available() -> None:
    final = np.array(["auto_action"] * 5 + ["human_review"] * 40 + ["allow"] * 55, dtype=object)
    correct = np.array([True] * 5 + [True] * 36 + [False] * 4 + [False] * 55)
    out = tier_metrics(final, correct, ("auto_action", "human_review", "allow"), 30)
    assert out["auto_action"]["precision"] is None
    assert "below the minimum" in out["auto_action"]["precision_note"]
...
def test_per_label_metrics_reports_ap_and_threshold_stats() -> None:
    y = np.array([[1], [0], [1], [0]])
    p = np.array([[0.9], [0.8], [0.7], [0.1]])
    out = per_label_metrics(
        y, p, ("toxic",), {"human_review": {"toxic": 0.7}, "auto_action": {"toxic": None}}
    )
    m = out["toxic"]
    ...
    assert m["at_auto_action"]["threshold"] is None
