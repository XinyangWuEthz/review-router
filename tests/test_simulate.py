...
def test_no_arrivals_have_no_completion_ratio_or_timing_samples() -> None:
    scenario = Scenario(0, 0.0, np.zeros(0, dtype=int), np.zeros(0), np.zeros(0))
    out, records = simulate(
        scenario, np.zeros(1), np.ones(1), np.ones(1, dtype=bool), CFG, "fifo"
    )
    assert len(records) == 0 and out.n_arrivals == out.n_handled == 0
    assert out.completion_ratio is None
    assert out.wait_p50 is None and out.completion_latency_p50 is None
    assert out.reviewer_utilization == 0.0


def test_saved_csv_record_flags_preserve_false_and_unfinished_status() -> None:
    _, records = simulate(
        _scenario([0.0, 59.0], [0, 1]),
        np.zeros(2),
        np.array([0.0, 5.0]),
        np.array([False, True]),
        CFG,
    ...
    rows = [
        ...
        for row in records.rows(strategy="fifo")
    ]
    rebuilt = JobRecords.from_rows(rows, CFG.horizon_hours * 60)
    assert rebuilt.high_risk.tolist() == [False, True]
    assert rebuilt.status.tolist() == ["completed", "in_progress"]
    assert time_metrics(rebuilt) == time_metrics(records)
